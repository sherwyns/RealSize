$( document ).ready(function() {
    ...
    $('#sizeinrealitysubmit').live('click', function(){   
    ...
    $('#deleteAr').live('click', function(){
        ...
        deleteProd (sizeInRealitytId, productid, model);
    });
...
function deleteProd(sizeInRealitytId, productid, model){
    var url = $('#getUrl').val();
    console.log(url);
    swal({
        title: 'Are you sure?',
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            xhr.open('POST', url, true);
            xhr.onload = function () {
          //  var data = "sizeInRealitytId=sizeInRealitytId&productid=productid&model=model";    
              if (xhr.readyState == 4 && xhr.status === 200) {
                var data = JSON.parse(xhr.responseText);
                //console.log(data);
                if(data.status == 1){
