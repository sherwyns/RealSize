$( document ).ready(function() {
    window.adminArView = new AdminArView();
});

var AdminArView = function(){
    this.init();
}

AdminArView.prototype = {
    file:null,
    page:null,
    inputTemp:null,
    arViewTable:null,
    ...
    init: function(){
        ...
        $('#arviewsubmit').on('click', function(){
        ...
        //--------------------------------------------------------------------------------------- End Of arviewsubmit
        $('#arviewupload').live('click', function(){   
        ...
        $('#deleteAr').live('click', function(){
            ...
            self.deleteFile (arViewtId, productid, model);
        });
        //--------------------------------------------------------------------------------------- End Of arviewdelete   
    ...
    dataTable: function(column0 = null, column1 = null){
        ...
        var arTable = $('#arViewTable').DataTable({
            "destroy": true,
            "processing": true,
            "serverSide": true,
            "ajax":{
                ...
                error: function(){ 
                    $(".arViewTableError").html("");
                    $("#arViewTable").append('<tbody class="arViewTableError"><tr><th colspan="3">No data found in the server</th></tr></tbody>');
                }
            },
                columnDefs: [
        ...
        fnRowCallback: function  (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                $('td:eq(3) .deleteProd', nRow).on('click', function () {
                    self.deleteFile(aData.arviewid, aData.productid, aData.model);
                });            
            },
        });
    },
    deleteFile: function(arViewtId, productid, model){
        var self = this;
        swal({
            title: 'Are you sure?',
        ...
        xhr.onload = function () {
          if (xhr.readyState == 4 && xhr.status === 200) {
                ...
                if(res.status == 1){
                    var data = res.data;
                    if(data != null){
                        console.log(self.arViewTable); 
                        if(self.page == 'arview'){
                            //self.arViewTable.ajax.reload();
                            self.resetForm();
    ...
    buildInfo: function(data){
        ...
        table += '<td> <a style="cursor:pointer;" data-id="'+ data.arviewid +'" data-model="' + data.file +  '" data-productid="'+ data.id_product +'" id="deleteAr">';
