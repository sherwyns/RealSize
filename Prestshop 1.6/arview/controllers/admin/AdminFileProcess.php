...
class AdminFileProcess 
{
    ...
    public static function processFile($fileName, $fileTmpName, $fileType, $id_product)
    {
        ...
        try{
            ...
            $fileSavePath = realpath(dirname(__FILE__).'/../..').'/AR/model/';
            ...
            if(!file_exists($modelPath)) {
                ...
                if($count == 3){
                    AdminArViewModel::insertDetails($id_product, $modelName); 
    ...
    {
        if($dir == null){
            $getmodel = realpath(dirname(__FILE__).'/../..').'/AR/model';
            $model = $getmodel;
        } else {
            $getmodel = $dir;
        }    
    ...
    public function deleteModel($arViewtId, $productid, $model)
    {
        if(!AdminArViewModel::deleteData($arViewtId, $productid))
            return ['status' => 0, 'message' => 'Oops Something went wrong!'];            
        $filePath = realpath(dirname(__FILE__).'/../..').'/AR/model/'.$model;
